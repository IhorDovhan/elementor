...
import { type AngleUnit, type LengthUnit, type TimeUnit } from '../../utils/size-control';
...
const mockTimeUnits = (): TimeUnit[] => [ 's', 'ms' ];
...
describe( 'SizeControl', () => {
	...
	describe( 'Time Units Support', () => {
		it( 'should handle time unit keyboard shortcuts', () => {
			// Arrange.
			const setValue = jest.fn();
			const props = { setValue, value: mockSizeProp( { size: 200, unit: 's' } ), bind: 'select', propType };

			// Act.
			renderControl( <SizeControl variant="time" units={ mockTimeUnits() } disableCustom />, props );

			const sizeInput = screen.getByRole( 'spinbutton' );

			fireEvent.keyUp( sizeInput, { key: 'm' } );
			expect( setValue ).toHaveBeenCalledWith( { $$type: 'size', value: { size: 200, unit: 'ms' } } );

			fireEvent.keyUp( sizeInput, { key: 's' } );
			expect( setValue ).toHaveBeenCalledWith( { $$type: 'size', value: { size: 200, unit: 's' } } );
		} );
	} );
} );
